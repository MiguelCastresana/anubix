...
anubix_clustering = function(network,
                             links_matrix,
                             genesets,
                             pathways,
                             cores = 2,
  ...
  final[, 6] = final[, 6]/genes_in_network
  
  # threshold to select genes
  ...
  if(nrow(genesets)<1){return("No modules were found")}else{
    ...
    if (length(length_genesets_filtered) > 100) {
      set = seq(from = 100, to = length(length_genesets_filtered), 
                by = 100)
    ...
    if(nrow(filtered_genesets)<1){
      ...
      if (website == TRUE) {
        ...
        anubix[,2] = sapply(anubix[,2], toupper)
