#' Network clustering of the gene set and then ANUBIX is applied
...
#' }
#'
#' @seealso \code{\link{anubix}},\code{\link{anubix_constrained}},\code{\link{anubix_links}},\code{\link{example_anubix}}
#'
#'
...
anubix_clustering_website = function(network,
                             links_matrix,
                             genesets,
                             pathways,
                             cores = 2,
  ...
  if (length(unique(as.vector(genesets[, 1])))<1)
    stop("ERROR: None of the genes are in the network")
  ...
  for(i in 1:length(group_sets)){
    ...
    sub_net = net[which((net[,1]%in%as.vector(sub[,1]) & net[,2]%in%as.vector(sub[,1])) | (net[,2]%in%as.vector(sub[,1]) & net[,1]%in%as.vector(sub[,1]))),]
    genesetd = graph_from_data_frame(sub_net, directed = FALSE, vertices = NULL)
    prueba = cluster_infomap(genesetd, e.weights = as.vector(sub_net[,3]))
    
    
    
    # if(length(as.vector(unlist(module)))<3){break}
  ...
  if (!is.null(callback)) {
    callback("20% of the process done")
  }
  
  
  names(keggclusters)=group_sets
  ...
  for(i in 1:length(group_sets)){
    if(groups[i]==0){
      ...
      for(k in 1:groups[i]){
        ...
        pathgroup=rep(paste("Module ",w,"_#",nombre,sep=""),n)
  ...
  modules1[,3] = gsub('(.*)#', '',modules1[,2])
  
  names(modules1) = c("gene","module","geneset")
  # genesets =   modules1[modules1[,2] %in% names(which(table(modules1[,2]) > 2)), ]
  ...
  names(freq) = c("geneset","SIZE_GENESET")
  ...
  final = left_join(modules1,freq,by = "geneset")
  
  
  freq  = as.data.frame(table(as.vector(modules1[,2])))
  
  names(freq) = c("module","SIZE_MODULE")
  
  final = left_join(final,freq,by = "module")
  
  
  final[,6] = final[,5]/final[,4]
  
  
  final = final[which(final[,6]>0.02),]
  
  genesets = modules1[which(modules1[,2]%in%unique(as.vector(final[,2]))),]
  
  if(nrow(genesets)<1){stop("ERROR: Non of the genes are in the network")}else{
    ...
    stat_computation = function(c, observed1, max1) {
      ...
      anubix[,1] = gsub('(.*)#', '',anubix[,1])
      
      anubix[,1]=trimws(anubix[,1], which = "left")
      
      
      
      
      parejas=anubix[,1:2][!duplicated(anubix[,1:2]),]
      k = 1
      filtered_anubix = data.frame()
      for(k in 1:nrow(parejas)){
        
        hits=anubix[which(anubix[,1]%in%parejas[k,1] & anubix[,2]%in%parejas[k,2]),]
        ...
        if(length(hits[,1])==0){
          ...
          pos=which(hits[,6]==valor)
          filtered_anubix = rbind(filtered_anubix,hits[pos[1],])
        }
    ...
    output_list = list()
    output_list[[1]] = genesets[,c(1,2)]
    output_list[[2]] = filtered_anubix
    return(output_list)
  }
